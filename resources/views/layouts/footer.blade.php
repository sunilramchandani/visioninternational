<link rel="stylesheet" href="{{ asset('css/footer.css') }}">

<div class = "col-lg-12 col-md-12 col-sm-12 col-xs-12 footer">
    <div class = "row top-footer-content">
        <div class = "col-lg-3 col-lg-offset-1 col-md-3 col-md-offset-1 col-sm-8 col-sm-offset-4 col-xs-8 col-xs-offset-4 vip-logo">
            <img src = "{{ URL::asset('image/footer-logo.png')}}" class = "img img-responsive footer-logo">
        </div>
        <div class = "col-lg-2 col-md-2 col-sm-3 col-xs-3 company">
            <p>COMPANY</p>
            <ul>
                <li>Opportunities</li>
                <li>About</li>
                <li>News</li>
                <li>Events</li>
                <li>Blog</li>
            </ul>
        </div>
        <div class = "col-lg-2 col-md-2 col-sm-3 col-xs-3 resources">
            <p>COMPANY</p>
            <ul>
                <li>Testimonials</li>
                <li>Hiring Partners</li>
                <li>Media</li>
            </ul>
        </div>
        <div class = "col-lg-2 col-md-2 col-sm-3 col-xs-3 support">
            <p>SUPPORT</p>
            <ul>
                <li>Job</li>
                <li>FAQ</li>
                <li>Contact Us</li>
            </ul>
        </div>
        <div class = "col-lg-2 col-md-2 col-sm-3 col-xs-3 Business">
            <p>BUSINESS</p>
            <ul>
                <li>Partner With Us</li>
            </ul>
        </div>
    </div>
    <div class ="col-lg-12 col-md-12 col-sm-12 col-xs-12 line-divider">
        <hr>
    </div>
    <div class = "col-lg-12 col-md-12 col-sm-12 bottom-footer-content">
        <div class = "col-lg-3 col-lg-offset-1 col-md-3 col-md-offset-1 col-sm-3 col-sm-offset-1 col-xs-3 col-xs-offset-1">
            <p>Crafted by Sky Rocket PH</p>
        </div>
        <div class = "col-lg-6 col-lg-offset-2 col-md-6 col-md-offset-2 col-sm-6 col-sm-offset-2 col-xs-6 col-xs-offset-2">
            <p>&copy; 2017, Vision International Philippines All rights reserved.   Privacy Policy. Terms of Service </p>
        </div>
    </div>
</div>