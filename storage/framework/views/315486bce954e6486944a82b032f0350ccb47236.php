<link rel="stylesheet" href="<?php echo e(asset('css/prefooter.css')); ?>">

<div class = "prefooter">
    <div class = "row top-prefooter-content text-center">
        <p class = "banner-paragraph">Start an amazing future with us! </p> 
        <a class = "btn applynow-button" href = "#"> Apply Now</a>
    </div>
    <div class = "row bottom-prefooter-content text-center">
        <p class = "subscribe-paragraph">Get the latest updates and news: </p>
        <input type = "email" class = "input-email-subscribe" name = "subscribe-email" placeholder="Enter your email address">
        <a class = "btn subscribe-button"><i class="fa fa-1x fa-angle-right" aria-hidden="true"></i></a>
    </div>
</div>